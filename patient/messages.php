...
// Fetch unique doctors the patient has appointments with, showing the latest one
$stmt = $db->prepare("
    SELECT a.id as apt_id, d.id as doctor_id, d.name as doctor_name,
           (SELECT message FROM chat_messages WHERE appointment_id IN (SELECT id FROM appointments WHERE patient_id = ? AND doctor_id = d.id) ORDER BY created_at DESC LIMIT 1) as last_msg,
           (SELECT created_at FROM chat_messages WHERE appointment_id IN (SELECT id FROM appointments WHERE patient_id = ? AND doctor_id = d.id) ORDER BY created_at DESC LIMIT 1) as last_time
    FROM (
        SELECT MAX(id) as id, doctor_id 
        FROM appointments 
        WHERE patient_id = ? 
        GROUP BY doctor_id
    ) latest_apts
...
<div class="flex h-[calc(100vh-140px)] bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden" x-data="{
    ...
    <div class="w-80 border-r border-slate-50 flex flex-col bg-slate-50/30">
        <div class="p-6 border-b border-slate-50">
            <h3 class="text-sm font-black text-slate-900 uppercase tracking-widest">My Doctors</h3>
        </div>
        <div class="flex-1 overflow-y-auto">
            <?php foreach ($conversations as $conv): ?>
                <button @click="selectConversation(<?php echo $conv['apt_id']; ?>, '<?php echo e($conv['doctor_name']); ?>')"
                    ...
                    <div class="w-12 h-12 bg-teal-50 text-teal-600 rounded-2xl flex items-center justify-center font-black shrink-0 group-hover:scale-110 transition-transform">
                        <?php echo strtoupper(substr($conv['doctor_name'], 0, 1)); ?>
                    </div>
                    <div class="overflow-hidden">
                        <p class="font-bold text-slate-900 text-sm truncate"><?php echo e($conv['doctor_name']); ?></p>
                        <p class="text-[10px] font-medium text-slate-400 mt-1 truncate">
                            <?php echo $conv['last_msg'] ? e($conv['last_msg']) : 'Start a conversation...'; ?>
                        </p>
                    </div>
                </button>
            <?php endforeach; ?>
    ...
    <div class="flex-1 flex flex-col bg-white">
        <template x-if="!activeApt">
            <div class="flex-1 flex flex-col items-center justify-center text-center p-10">
                ...
                <h3 class="text-lg font-black text-slate-900 tracking-tight">Your Inbox</h3>
                <p class="text-slate-400 text-sm font-medium mt-2">Select a doctor from the left to view your consultation history.</p>
            </div>
        </template>
                ...
                <div class="p-6 border-b border-slate-50 flex items-center justify-between bg-white">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Clinical Consultation</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 border-t border-slate-50">
                    ...
                    <div class="relative flex items-center gap-3">
                        ...
                        <div class="flex-1 relative">
                            <input type="text" x-model="newMessage" @keydown.enter="sendMessage()" placeholder="Reply to your doctor..."
